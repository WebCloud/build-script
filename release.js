...
// This should only be passed in by an automated script. So unless you are pretty sure of what you are doing, don't use it 🙃
if (updateMaster) {
  commandPromise = commandPromise.then(executeWithMessage('Updating git origin/master', 'git push origin HEAD:master'));
} else {
  log('📦  Hello human! You chose to release from your machine instead of an automated script. 📦', yellow);
